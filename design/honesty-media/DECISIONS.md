...
| **D03** | How the reconstruction worker writes verdicts and artifact pointers | (a) a fourth database role (`scan_worker`) with EXECUTE on the `app.record_scan_*` functions only; (b) the worker calls an authenticated HTTP endpoint (`/api/scan-worker/*`, bearer `SCAN_WORKER_SECRET`, same shape as `/api/cron/*`) which runs as `app_user` and calls the `SECURITY DEFINER` functions | **(b).** It preserves the "three roles, never collapse them" invariant in `CLAUDE.md`, reuses the cron/webhook pattern, keeps the worker without any database credential, and lets the same worker run on a laptop, a container, or later Visual Rails without a Neon role per environment. Idempotency: the callback carries the scan id and a job attempt; the function refuses a stale attempt. **As built (HM-03):** `app.claim_next_scan_job` / `app.finish_scan_job` / `app.retry_scan_reconstruction` / `app.release_stale_scan_jobs` in `drizzle/0017`; `worker/` holds `S3_*` for the footage and `SCAN_WORKER_SECRET`, nothing that reaches Postgres. | HM-03, decided in HM-02's contract |
...
| **D14** | Completion notifications | None / email / push | **Email** through the existing provider (`server/lib/email.ts`), one message per terminal state (`needs_mask`, `verified`, `rejected`, `failed`), subject lines in [HM-D04](screens/HM-D04.md). No new provider. **As built (HM-03):** `needs_mask` and `failed` are sent from the worker's finish route (and `failed` from the stale-release cron), `rejected` from the HM-02 completion route via `app.scan_notice`; the `verified` mail ships with the ticket that can set `verified` (HM-04/05). Always after the transaction has committed, never inside it. | HM-03 |
...
| `GET /listings/:id` (`street` block) | public (owner for own drafts) | **As built (HM-06):** the pin at the precision this viewer is entitled to, `precisionM`, `hasApproach`, and a signed approach poster only for a viewer who may see the door. No separate route: the section is part of the page, and a second endpoint would be a second place to get the entitlement wrong | HM-D08, D09 |
...
| `GET`/`POST /cron/release-stale-scan-jobs` | scheduler (bearer `CRON_SECRET`) | **As built (HM-03):** a claim older than `scan_claim_stale_hours` returns to the queue, or fails once `scan_max_attempts` is spent (host emailed) | HM-03 |
| `GET /listings/:id/scans/:scanId/stills` | owner | **As built (HM-03):** up to 8 short-lived signed URLs to the worker's real frames, for the failed-state page | HM-D04 |
