...
| Policy version | `HONESTY_POLICY_VERSION = 1` in the copy module (HM-00 adds no migration, per BUILD-PLAN's HM-00 file map); HM-01's migration (`drizzle/0015`) adds the `app_config` key and snapshots it onto each scan row at creation | A later copy change is visible as a version, not a silent rewrite |
