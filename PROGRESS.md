...
| HM-02 Upload package + storage | `claude/hm-02-scan-upload` | `drizzle/0016`: `scan_artifacts`, `scan_geo_samples` (no member grants), `app.complete_scan_upload`. Multipart video + JSON record + notes under the scan prefix; completion judges the record server-side → `uploaded` / `rejected`. Capture page uploads after Finish. No worker, no viewer, kill-switch untouched. |
| HM-03 Reconstruction worker | `claude/hm-03-reconstruction-worker` | `drizzle/0017`: job columns on `listing_scans` (`attempt`, `claimed_at`, `worker_id`) and five `SECURITY DEFINER` functions (claim / finish / retry / release / notice). `/api/scan-worker/*` bearer routes (D03: the worker holds no DB credential), `/api/cron/release-stale-scan-jobs`, host retry + stills routes, `/host/listings/:id/scan/status` (HM-D04), one email per terminal state (D14). `worker/`: Node orchestrator over ffmpeg + COLMAP (via `ns-process-data`), splatfacto, `ns-export`, `splat-transform`; no generative stage, hard-avoid list refused by name. No mask, no viewer, kill-switch untouched. |
| HM-04 Private rooms mask | `claude/hm-04-private-rooms-mask` | `drizzle/0018`: `listing_rental_masks` (host-writable under RLS), `listing_scans.job`, `app.send_scan_for_verification`, and claim / finish / release / retry made job-aware. `/host/listings/:id/scan/mask` (HM-D05) marks private stretches of the walk or confirms whole home; the worker drops those frames before it trains, so a private room is absent rather than hidden. Verified email (D14) ships. No viewer, no publish gate, kill-switch untouched. |
| HM-05 Guest walkthrough viewer | `claude/hm-05-walkthrough-viewer` | `drizzle/0019`: `listings.scan_verified_at` + `verified_scan_id` (written only by the transitions; `app_user`'s UPDATE on `listings` becomes a column list), `app.walkthrough_for_viewer`. Public `GET /api/listings/:id/walkthrough` signs short-lived URLs for the mask-applied artifact and the stills; `/listing/:id/walk` renders it with Spark + Three.js in a lazy chunk, falling back to real frames without WebGL and defaulting to them under reduced motion. No approach, no publish gate, kill-switch untouched. |
| HM-06 The street: pin + precision | `claude/hm-06-street-and-pin` | `drizzle/0020`: `listings.approach_visibility` (host-writable, default `confirmed_stay`), `app.round_pin`, `app.viewer_sees_door`, `app.street_for_viewer`, `app_config.street_pin_precision_m`. Listing detail gains "The street" (HM-D08): a MapLibre pin on OpenFreeMap tiles (D08, no key) in a lazy chunk, the precision sentence, and the imagery label. The pin is rounded to a 150 m grid in the database for anyone but the host, a guest with a confirmed stay, or a home whose host opened it (D09). Host setting in the editor. **First half only:** no approach footage and no door tether — those wait on the worker slice. No publish gate, kill-switch untouched. |
