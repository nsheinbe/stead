...
* DB: `npm run db:migrate` · roles: `npm run db:bootstrap-roles` · seed: `ALLOW_DEMO_SEED=1 npm run db:seed` (refuses production) · new migration: add the next numbered file in `drizzle/`
...
* Without a send key (`POSTMARK_SERVER_TOKEN` / `POSTMARK_API_TOKEN`, or `RESEND_API_KEY`) the magic link prints to the server console. That is the intended local-dev path, not a bug. With a key, `AUTH_EMAIL_FROM` must be a verified-domain address — `onboarding@resend.dev` is refused (Gmail drops it). Stead production: Postmark free + `openstead.app` (`Stead <noreply@openstead.app>` or `Stead <hello@openstead.app>`). Prefer Postmark when both keys are set.
